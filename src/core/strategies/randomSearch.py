...
class SolucaoAspirador(Solucao):
	...
	def reproduzir(self, outro, crossoverProb, mutacaoProb, resolucao, nsujeira, nlixeira, ncarga, nagente):
		...
		#print novoIndividuo
		novoIndividuo.mutacao(mutacaoProb)
...
class RandomSearchAspirador(RandomSearch):
	def __init__(self, npop, maxite):
		super(RandomSearchAspirador, self).__init__(npop, maxite)
		self.nagentes = 0
		self.resolucao = 0
		self.nsujeira = 0
		self.nlixeira = 0
	...
	def gerarIndividuo(self, resolucao, nsujeira, nlixeira, ncarga, nagentes):
		...
		individuo.objetivos = self.avaliador(individuo.decodificar(self.componentes)) #pausa longa
		return individuo
